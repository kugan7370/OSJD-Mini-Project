...
function JerseyCatergory() {
    ...
    useEffect(() => {
        if (categoryDatas) {
            const Jeseydetail = categoryDatas.filter((jersey) => jersey.category_id == id)
    ...
    return (
        <div className="jerycatergory px-8 mt-8">
            <div className="grid grid-cols-5 gap-10">
                {jerseyDetails && jerseyDetails?.map((jerseyDetail) => (
                        ...
                        <Link to={`/detail/Order/${jerseyDetail?._id}`}>
                            <div className="flex flex-col justify-center items-center  p-4 shadow-xl relative  ">
                                <div className="p-4 w-[180px] h-[220px] ">
                                    <img className='w-full h-full hover:scale-[110%] duration-500' src={jerseyDetail?.image} alt="" />
                                </div>
                                <span className='uppercase font-semibold text-center mt-4 '>{jerseyDetail.type}</span>
